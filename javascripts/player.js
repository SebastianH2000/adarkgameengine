...
window.onload = function() {
    ...
    if (localStorage.getItem("save-file") === null) {
        player.energy = new Decimal("1e10");
        switchTab('energy');
...
function loadData() {
    ...
    safeLoadDataFast('currentTab');

    safeLoadDataFast('theme');
}

function safeLoadData(p1,p2,p3) {
    if (typeof p1[p2] !== "undefined") p3[p2] = p1[p2];
}

function safeLoadDataFast(p1) {
    safeLoadData(gameSave,p1,player);
}